...
locals {
  ...
  ecs_name       = "${local.usershort}-${local.env}-${local.aws_region}-ecs"
...
# Note that this module is on the PRIMARY environment rather than the current environment
# This means duplicate environments in the same region will automatically share a single VPC
dependency "vpc" {
